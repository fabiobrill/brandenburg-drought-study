...
means_per_crop = df %>% select(year, crop, LST, NDVI, LSTNDVI, AZL, TWI, NFK, area_iacs) %>%
                        group_by(crop) %>%
                        summarize(AZL_m = mean(AZL, na.rm=T),
...
anomalies = df %>% select(year, crop, LST, NDVI, LSTNDVI) %>% 
