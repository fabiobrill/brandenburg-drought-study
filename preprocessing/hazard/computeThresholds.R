    ...
    for(lkoi in unique(data$NUTS_NAME)){
        for(coi in unique(data$crop)){
            ...
            for(threshold in seq(0.25, 1.5, 0.25)){
                temp = data %>% 
                    select(year, NUTS_NAME, crop, area_iacs, LSTNDVI_anom) %>% 
                    dplyr::filter(year == yoi, NUTS_NAME == lkoi, crop == coi)
...
for(yoi in 2013:2022){
    for(lkoi in unique(data$NUTS_NAME)){
        for(coi in unique(data$crop)){
            ...
            specific_area = data %>% 
                dplyr::filter(year == yoi, NUTS_NAME == lkoi, crop == coi) %>%
                select(area_iacs) %>% sum(na.rm=T)
            for(threshold in c(0, 5, 10, 15, 20, 25, 30, 35)){
                temp = data %>% 
                    select(year, NUTS_NAME, crop, area_iacs, SMI_total) %>% 
                    dplyr::filter(year == yoi, NUTS_NAME == lkoi, crop == coi)
    ...
    for(lkoi in unique(data$NUTS_NAME)){
        for(coi in unique(data$crop)){
            ...
            for(threshold in seq(0, 0.15, 0.05)){
                temp = data %>%
                    select(year, NUTS_NAME, crop, area_iacs, SMI_magnitude,
    ...
    for(lkoi in unique(data$NUTS_NAME)){
        for(coi in unique(data$crop)){
            ...
            for(threshold in seq(-4, 0, 0.5)){
                temp = data %>%
                    select(year, NUTS_NAME, crop, area_iacs, SPEI_magnitude,
...
# ----------------------------------------------------------------------------------------------- #
# Ackerzahl, using the LBG thresholds
# ----------------------------------------------------------------------------------------------- #
impacted = data.frame()
    ...
    for(lkoi in unique(data$NUTS_NAME)){
        for(coi in unique(data$crop)){
            ...
            for(threshold in c(23, 29, 36, 46)){
                temp = data %>%
                    select(year, NUTS_NAME, crop, area_iacs, AZL) %>%
                    dplyr::filter(year == yoi, NUTS_NAME == lkoi, crop == coi)
    ...
    for(lkoi in unique(data$NUTS_NAME)){
        for(coi in unique(data$crop)){
            ...
            for(threshold in 1:5){
                temp = data %>%
                    select(year, NUTS_NAME, crop, area_iacs, LBG) %>%
                    dplyr::filter(year == yoi, NUTS_NAME == lkoi, crop == coi)
